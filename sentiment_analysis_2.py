...
#model.add(Embedding(maxDictionary_size, 32, input_length=maxWordCount)) #to change words to intsare platform for digital assets. Offering the largest production block chain platform in the world, we are usin
model.add(Embedding(maxDictionary_size,300,input_length=maxWordCount,trainable=True))
 #hidden layers
model.add(LSTM(100))
...
earlystop = EarlyStopping(monitor='val_acc',patience=20,verbose=1,mode='auto',min_delta=0.01)
